...
function loadConfig() {
  if (process.argv.length >= 2) {
    ...
    const config_default = JSON.parse(
      fs.readFileSync(path_config_default, { encoding: "utf8" })
    )
    const config_custom = JSON.parse(
      fs.readFileSync(path_config, { encoding: "utf8" })
    )
