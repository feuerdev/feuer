...
export default class GameClass {
  ...
  constructor(uid: string, world: World, renderer: Renderer) {
    ...
    EventBus.shared().on("gamestate tiles", ( detail ) => {
      console.log("hi")
      let tiles: Util.Hashtable<ClientTile> = detail as unknown as Util.Hashtable<ClientTile>
    ...
    EventBus.shared().on("gamestate relation", (detail) => {
      let relation: PlayerRelation.default = detail as unknown as PlayerRelation.default
  ...
  private trySelect(point: Vector2) {
    ...
    const hit = this.renderer.viewport.children.filter((sprite) => {
      return Util.isPointInRectangle(point.x, point.y, sprite.x, sprite.y, (<Sprite>sprite).width, (<Sprite>sprite).height)
    })
