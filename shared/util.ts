...
export function calculateHorizontalAngle(
  ...
  let result =
    radiansToDegrees(Math.atan2(to.y - from.y, to.x - from.x)) - orientation
  if (result < -180) {
...
export function calculateAimpoint(
  ...
  const d =
    (Math.pow(velocity, 2) / -gravity) *
    Math.sin(2 * degreeToRadians(angleVertical))
  const angle = angleHorizontal + orientation
  const vec: Vector2 = new Vector2(
    d * Math.cos(degreeToRadians(angle)),
    d * Math.sin(degreeToRadians(angle))
  )
