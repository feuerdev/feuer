...
export function astar(tiles, start:Hex, goal:Hex) {
  ...
  while(!frontier.isEmpty()) {
    ...
    for(let next of current.neighbors()) {
      if(tiles[next.hash()]) {
        let new_cost = cost_so_far[current.hash()] + 1/tiles[current.hash()].movementFactor; //TODO calculate correct movementcost
        if(cost_so_far[next.hash()] === undefined || new_cost < cost_so_far[next.hash()]) {
